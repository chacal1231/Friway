...
class GroupsController extends GroupAppController {
    ...
    public function view($id = null,$group_name = null,$invite_checksum = null) {
        ...
        if ($group['Group']['type'] != PRIVACY_PRIVATE || ($cuser && $cuser['Role']['is_admin'])) {
            $this->set('description_for_layout', String::truncate(strip_tags($group['Group']['description']), 160, array('ellipsis' => '...', 'html' => false, 'exact' => false)));
        }
        // set og:image
    ...
    public function ajax_change_admin($id, $type) {
        $this->autoRender = false;
        $this->loadModel('Group.GroupUser');
    ...
    public function categories_list(){
        if($this->request->is('requested')){
            $this->loadModel('Category');
            $categories = $this->Category->getCategories('Group');
            return $categories;
        }
    }
    ...
    public function my_joined_group() {
        $uid = $this->Auth->user('id');
        $this->loadModel('Group.GroupUser');
        $groups = $this->GroupUser->getJoinedGroups($uid, 10);
        $this->set(compact('groups'));
    }
