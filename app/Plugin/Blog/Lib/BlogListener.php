...
class BlogListener implements CakeEventListener {
    ...
    public function search($event)
    {
        ...
        {
            $event->result['Blog']['header'] = "Blogs";
            $event->result['Blog']['icon_class'] = "icon-edit";
    ...
    public function suggestion($event)
    {
        ...
        if(isset($event->data['type']) && $event->data['type'] == 'blog')
        {
            ...
            $e->set('blogs', $blogs);
            $e->set('result',1);
            $e->set('more_url','/search/suggestion/blog/'.$e->params['pass'][1]. '/page:' . ( $page + 1 ));
            ...
            if(!empty($blogs)){
                foreach($blogs as $index=>&$detail){
                    ...
                    if(!empty($detail['Blog']['thumbnail']))
                    {
                        ...
                        $event->result['blog'][$index]['img'] = 'blogs/thumbnail/'.$detail['Blog']['id'].'/75_square_'.$detail['Blog']['thumbnail'];

                    }
                    $event->result['blog'][$index]['title'] = $detail['Blog']['title'];
                    $event->result['blog'][$index]['find_name'] = 'Find Blog';
                    $event->result['blog'][$index]['icon_class'] = 'icon-edit-1';
    ...
    public function hashtags($event)
    {
        ...
        $items = $this->Blog->find('all', array('conditions' => array(
                'Blog.id' => $blog_ids
            ),
            'limit' => RESULTS_LIMIT,
            'page' => $page
        ...
        if(!empty($blogs))
        {
            $event->result['blogs']['header'] = 'Blogs';
            $event->result['blogs']['icon_class'] = 'icon-edit';
