					...
					<script>
						function cancelRecurring()
						{
							$.fn.SimpleModal({
								...
								var popup = this;
								$.post(baseUrl + "/subscription/subscribes/cancel_recurring", {text_reason:$('.simple-modal-body #reason').val()}, function(data){
									popup.hideModal();
									if (data.status)
									{
				...
				<?php if ($helper->canRefunded($subscribe)):?>
					...
					function requestRefund()
					{
						$.fn.SimpleModal({
							...
							$('.simple-modal-footer .btn.btn-action').addClass('disabled');
							$('.simple-modal-footer .btn.btn-action').spin('small');
							$.post(baseUrl + "/subscription/subscribes/request_refund", {account:$('.simple-modal-body #account').val(),reason:$('.simple-modal-body #reason').val()}, function(data){
								popup.hideModal();
								if (data.status)
								{
									window.location=window.location;
