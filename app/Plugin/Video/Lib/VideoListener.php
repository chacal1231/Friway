...
class VideoListener implements CakeEventListener {
    ...
    public function getGroupMenu($event) {
        $event->result['menu'][] = array(
            ...
            'icon-class' => 'icon-facetime-video',
            'name' => __('Videos'),
    ...
    public function search($event)
    {
        ...
        {
            $event->result['Video']['header'] = "Videos";
            $event->result['Video']['icon_class'] = "icon-facetime-video";
    ...
    public function suggestion($event)
    {
        ...
        if(isset($event->data['type']) && $event->data['type'] == 'video')
        {
            ...
            $e->set('videos', $videos);
            $e->set('result',1);
            $e->set('more_url','/search/suggestion/video/'.$e->params['pass'][1]. '/page:' . ( $page + 1 ));
            ...
            if(!empty($videos)){
                foreach($videos as $index=>&$detail){
                    ...
                    if(!empty($detail['Video']['thumb']))
                        $event->result['video'][$index]['img'] = 'videos/thumb/'.$detail['Video']['id'].'/75_square_'.$detail['Video']['thumb'];
                    $event->result['video'][$index]['title'] = $detail['Video']['title'];
                    $event->result['video'][$index]['find_name'] = 'Find Videos';
                    $event->result['video'][$index]['icon_class'] = 'icon-facetime-video';
    ...
    public function hashtags($event)
    {
        ...
        $items = $this->Video->find('all', array('conditions' => array(
                'Video.id' => $video_ids
            ),
            'limit' => RESULTS_LIMIT,
            'page' => $page
        ...
        if(!empty($videos))
        {
            $event->result['videos']['header'] = 'Videos';
            $event->result['videos']['icon_class'] = 'icon-facetime-video';
