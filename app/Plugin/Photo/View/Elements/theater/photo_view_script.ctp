    ...
    if ( uid != '' || $("#tag-name").val() != '' )
    {
        ...
        $.post( '<?php echo $this->request->base?>/photos/ajax_tag', {photo_id: photo_id, uid: uid, value: $("#tag-name").val(), style: style}, function( data ){
            ...
            if ( json.result == 1 )
            {
                ...
                $("#tags").append('<span id="hotspot-item-' + tagCounter + '" onmouseover="showTag(' + tagCounter + ')" onmouseout="hideTag(' + tagCounter + ')">' + tagValue + '<a href="javascript:void(0)" onclick="removeTag(' + tagCounter + ', ' + json.id + ')"><i class="icon-delete cross-icon-sm"></i></a></span>');
