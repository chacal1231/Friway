...
<div id="photo_wrapper" >
    ...
    <div id="tag-wrapper">
        <img src="<?php echo $photoHelper->getImage($photo, array('prefix' => '1500'));?>" onload="showPhotoWrapper()" id="photo_src">
 ...
 ?>
    ...
    <a href="<?php echo $this->request->base?>/albums/view/<?php echo $photo['Photo']['target_id']?>/<?php echo seoUrl($photo['Album']['title'])?>" id="photo_close_icon" class="lb_icon"><i class="icon-delete icon-2x topButton"></i></a>
    <?php endif; ?>

    <?php if (!empty($neighbors['next']['Photo']['id'])): ?>
    <a href="javascript:void(0)" onclick="showPhoto(<?php echo $neighbors['next']['Photo']['id']?>)" id="photo_left_arrow" class="lb_icon"><i class="icon-left-open-big icon-4x"></i></a>
    <?php endif; ?>
    
    <?php if (!empty($neighbors['prev']['Photo']['id'])): ?>
    <a href="javascript:void(0)" onclick="showPhoto(<?php echo $neighbors['prev']['Photo']['id']?>)" id="photo_right_arrow" class="lb_icon"><i class="icon-right-open-big icon-4x"></i></a>
    <?php endif; ?>
    <?php
        $nextPhoto = '';
 ...
 ?>
                ...
                <div id="tags" style="margin:5px 0;">
                    ...
                    <span onmouseout="hideTag('0-<?php echo $tag['PhotoTag']['id']?>')" onmouseover="showTag('0-<?php echo $tag['PhotoTag']['id']?>')" id="hotspot-item-0-<?php echo $tag['PhotoTag']['id']?>">
                        ...
                        ?><a onclick="removeTag('0-<?php echo $tag['PhotoTag']['id']?>', <?php echo $tag['PhotoTag']['id']?>)" href="javascript:void(0)"><i class="icon-delete cross-icon-sm"></i></a>
                        <?php
                        endif;
                        ?>
                    </span>
                ...
                <div class="box4">
                    <ul class="photo-view-option">      
                        ...
                        <li><a href="javascript:void(0);" onclick="set_photo_as_cover(<?php echo $photo['Photo']['id']?>);" class=""><?php echo __( 'Set as cover')?></a><span id="set_cover"></span></li>
                        <li><a href="javascript:void(0);" onclick="set_photo_as_profile_picture(<?php echo $photo['Photo']['id']?>);" class=""><?php echo __( 'Set as profile picture')?></a><span id="set_avatar"></span></li>
                        <?php endif; ?>
                        
                        <?php if ( ( $uid && $cuser['Role']['is_admin'] ) || ( !empty( $admins ) && in_array( $uid, $admins ) ) ): ?>

                            <li><a href="javascript:void(0)" onclick="deletePhoto(<?php echo  $nextPhoto; ?>)"><?php echo __( 'Delete Photo')?></a></li>
                        <?php endif; ?>
                        <li>
                            <?php
      $this->MooPopup->tag(array(
