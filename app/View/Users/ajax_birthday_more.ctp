...
$(document).ready(function(){
    ...
    $('.postFriendWall').click(function(){
        ...
        if ($.trim(msg) != '')
        {
            disableButton('status_btn_'+id);
            $.post("<?php echo  $this->request->base; ?>/activities/send_birthday_wish", $("#wallForm_"+id).serialize(), function(data){
                var json = $.parseJSON(data);
                enableButton('status_btn_'+id);
                $('#message_'+id).val("");
                if (json.success)
                {
                    enableButton('status_btn_'+id);
                    me.parent(".birthday-wish").html("<div style='padding:5px 0px;'><?php echo addslashes(__('Birthday wish is sent')); ?></div>");
                }
            });
        }
    })
                ...
                <div class="more p_l_60">
                    ...
                    <div>
                        <form id="wallForm_<?php echo  $u['User']['id']; ?>">
                            ...
                            $text = "Write on ".$u['User']['name'] ."'s timeline";
                            ...
                            ?>
                                <div class="birthday-wish">
                                    ...
                                    <?php
                                    else: echo "Birthday wish is sent";
                                        endif;
                                    ?>
                                </div>
