...
class MooHelper extends AppHelper {
    ...
    public function getItemPhoto($obj, $options_link = array(), $options_image = array(),$linkOnly=false) {
        ...
        if (key($obj) == 'User') {
        	if (!$obj['User'])
        	{
                ...
                $obj[key($obj)]['moo_title'] = __('No login');
                return $this->Html->image($thumbUrl, array_merge($options_image, array('alt' => h($obj[key($obj)]['moo_title']), 'title' => h($obj[key($obj)]['moo_title']) ) ) );
        	} 	
        }
        ...
        return $this->Html->link($this->Html->image($thumbUrl, array_merge($options_image, array('alt' => h($obj[key($obj)]['moo_title']) , 'title' => h($obj[key($obj)]['moo_title']) ) ) ), FULL_BASE_URL . $obj[key($obj)]['moo_href'], array_merge($options_link, array('escape' => false)));
    }
        ...
        if ($thumb) {
            ...
            if (key($obj) == 'User') {
                if (isset($options['prefix']) && $options['prefix'] == '50_square') {
                    $url = FULL_BASE_URL . $this->request->webroot . strtolower(key($obj)) . '/img/noimage/' . $obj[key($obj)]['gender'] . '-' . strtolower(key($obj)) . '-sm.png';
                } else {
                    $url = FULL_BASE_URL . $this->request->webroot . strtolower(key($obj)) . '/img/noimage/' . $obj[key($obj)]['gender'] . '-' . strtolower(key($obj)) . '.png';
                }
            } else {
    ...
    public function getName($userdata, $bold = true, $blank = false) {
        if (!empty($userdata)) {
            ...
            if ($bold)
                return '<a ' . $target . ' href="' . $url . '"><b>' . $name . '</b></a>';
            else
    ...
    public function cleanHtml($text) {
        $text = strip_tags($text, '<span><ul><ol><li><img><a><p><br><b><i><u><strong><em><sub><sup><div><blockquote>');
        $text = $this->parseSmilies($text);
        ...
        if ($menus != null) {
            switch ($menu_type) {
                ...
                case 'vertical':
                    ...
                    $menuHtml .= '<li ' . $active . '>' . $this->Html->Link('<i class="spicon-puzzle"></i> ' . $plugin['Plugin']['name'], reset($menus), array('escape' => false)) . '</li>';
                    break;
            }
        }
        return $menuHtml;
