...
class MooHtmlHelper extends HtmlHelper{
    ...
    public function __construct(View $View, $settings = array()) {
        if ($View instanceof MooView) {
            $this->isActiveRequirejs = $View->isEnableJS("Requirejs"); //var_dump($this->isActiveRequirejs); die();
        }

        parent::__construct($View,$settings);
        $this->html['viewMore']=<<<HTML
<div class="view-more">
        <a href="javascript:void(0)" onclick="moreResults('%s', '%s', this)">%s</a>
</div>
HTML;
    }
    private $html = array();
    ...
    public function script($url, $options = array()) {
        if($this->isActiveRequirejs){
            ...
            foreach ($url as $u) {
                $tmp_id = uniqid();
                $this->MooRequirejs->addPath(array($tmp_id => $this->MooRequirejs->assetUrlJS($u)));
                $this->MooRequirejs->addToFirst(array($tmp_id));
            }
    ...
    public function viewMore($moreUrl,$div='list-content',$text=null,$autoEcho=true){
        if(!$this->isLoaded['javascript']['behavior.js']){
            ...
            //$this->_View->requireJs('moocore/behavior.js','mooBehavior.initAutoLoadMore();');
        }

        if(empty($text)) $text=__( 'Load More');
