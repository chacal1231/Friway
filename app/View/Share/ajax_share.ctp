<?php
echo $this->Html->script(array('vendor/jquery.hideshare'), array('inline' => false));
?>
...
<?php endif; ?>
    $(document).ready(function () {
        ...
        $(".sharethis").hideshare({
            linkedin: false,
            link : "<?php echo $social_link_share; ?>"
        });
        ...
        switch (activeTab) {
            case "#friend":
                if ($('#friendSuggestion').val() == ''){
                    $('#shareWarning').html('<span><?php echo addslashes(__("Please select friends to share.")); ?></span>');
                    return;
                }
                break;
            case "#msg":
            ...
            case "#group":
                if ($('#groupSuggestion').val() == ''){
                    $('#shareWarning').html('<span><?php echo addslashes(__("Please select groups to share.")); ?></span>');
                    return;
                }
                break;
            case "#email":
                if ($('textarea[name="data[email]"]').val() == ''){
                    $('#shareWarning').html('<span><?php echo addslashes(__("Please input emails to share.")); ?></span>');
                    return;
                }
                break;
            default:
...
?>",
                ...
                function (data) {
                    ...
                    window.parent.statusModal('<?php echo __('Status'); ?>', $parseJson.msg);
                }
        );
    });
        ...
        <div class="tab-content share_content">
            ...
            <div role="tabpanel" class="tab-pane fade" id="email">
                ...
                <div class="suggestion shareViaEmail" style="display:block;">
                    <?php echo $this->Form->textarea('email', array('placeholder' => __('separate email by comma')), false, false); ?>
                </div>
            </div>
            ...
            <div class="userTagged" style="display:block">
                ...
                <div class="stt-action share-action">
                    ...
                    <div class="user-tagging-container">
                        <i class="icon-user-add" data-toggle="tooltip" title="<?php echo __('Tag people in your post'); ?>" onclick="$('.userTagging-userTaggingOnShareForm').toggleClass('hidden')"></i>
                    </div>
