...
<?php echo $this->Form->hidden('id', array('value' => $role['Role']['id']))?>
    <div class="form-body">
        ...
        <?php foreach ( $aco_groups as $group => $acos ): ?>
            ...
            <div class="form-group">
                <div class="col-md-12">
                    <label><?php echo  __d('permission',Inflector::humanize($group)) ?> - <?php echo __('Permissions');?></label>

                    <div class="checkbox-list">


                        <?php foreach ($acos as $aco): ?>
                            <label><?php echo  $this->Form->checkbox('param_' . $group . '_' . $aco['Aco']['key'], array('checked' => in_array($group . '_' . $aco['Aco']['key'], $permissions))) ?> <?php echo  __d('permission',$aco['Aco']['description']) ?></label>
                        <?php endforeach; ?>
                    </div>
                </div>
            </div>
