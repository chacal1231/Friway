...
if ( !( empty($uid) && Configure::read('core.force_login') ) ):
    ...
    //if(empty($member_only)) $member_only = 1;
    ...
    <div class="box2">
        <?php if($title_enable): ?>
            <h3><a href="<?php echo $this->request->base?>/users/index/online:1">
                    <?php if(empty($title)) $title = "Who's Online";?>
                    <?php echo $title; ?>
                </a>
            </h3>
        <?php endif; ?>
            ...
            <?php if ( !empty( $online['members'] ) ): ?>
                <ul class="list_block">
                    <?php foreach ($online['members'] as $u): ?>
                        <li>
                                <?php echo $this->Moo->getItemPhoto(array('User' => $u['User']), array( 'prefix' => '50_square'), array('class' => 'img_wrapper2 user_avatar_large tip'))?>
                        </li>
                    <?php endforeach; ?>
                </ul>
            <div class='clear'></div>
