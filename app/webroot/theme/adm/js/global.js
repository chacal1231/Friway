...
$(document).ready(function(){	
    ...
    jQuery('button#login_as_user').on('click',function(e){
        e.preventDefault();
        var id = $(this).data('id');
        mooConfirm(MooPhrase.__('confirm_login_as_user'),baseUrl + '/admin/users/login_as_user/'+id);
    });
});
...
function doModeration( action, type )
{
    ...
    {
        case 'delete':
            $('#deleteForm').attr('action', baseUrl + '/admin/' + url + '/delete');
            confirmSubmitForm('Are you sure you want to delete these ' + type + '?', 'deleteForm'); 
        break;
        
        case 'move':
            $('#deleteForm').attr('action', baseUrl + '/admin/' + url + '/move');
            $('#category_id').show();
        break;
