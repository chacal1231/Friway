...
//                if(avaiableBlocks == 62)//drop tabs widget
//                {
...
//                    var last_index = $(this).find('.dd-list:first').children('li').length;
//                    $($(this).find('.dd-list')[0]).find('li:last div').css({'height':'auto'});
...
//                    $($(this).find('.dd-list')[0]).find('li:last div').html('');
//                    $($(this).find('.dd-list')[0]).find('li:last div').append('<div class="tabs-title">' +
//                        '<span>'+text+'</span> ' +
//                        '<i class="fa fa-times pull-right delete-element" style="cursor:pointer; margin-top:3px;"></i> ' +
//                        '<a href="javascript:void(0)" class="edit-block pull-right" data-block-id="'+avaiableBlocks+'">' +
//                        '   <i class="fa fa-pencil"></i>' +
//                        '</a>' +
//                        '</div>' +
//                        '<div id="'+$(event.target).attr('id')+'-tabs-container-'+last_index+'" class="tabs-container ui-state-default"  style=" border: 1px solid #ffaa00"><ol class="dd-list" style="min-height:50px;"></ol></div>');
//                    var a = $($($(this).find('.dd-list')[0]).find('li:last div').find('a')[0]);
//                    initDragDrop('#'+$(event.target).attr('id')+' .tabs-container','#'+$(event.target).attr('id'));
//                }
//                else if(avaiableBlocks == 63)//drop carousel widget
//                {
//                    $(this).find(".placeholder").remove();
//                    droppedWidget = $(ui.draggable.clone()).appendTo($(this).find('.dd-list')[0]);
//                    var last_index = $(this).find('.dd-list:first').children('li').length;
//                    $($(this).find('.dd-list')[0]).find('li:last div').css({'height':'auto'});
//                    //$($(this).find('.dd-list')[0]).find('li:last div').attr('id','#'+$(event.target).attr('id')+'-carousel-container-'+last_index);
//                    $($(this).find('.dd-list')[0]).find('li:last').attr('data-display-name',blocks[avaiableBlocks][1]);
//                    $($(this).find('.dd-list')[0]).find('li:last').attr('data-id','0');
//                    var text = $($(this).find('.dd-list')[0]).find('li:last div').html();
//                    $($(this).find('.dd-list')[0]).find('li:last div').html('');
//                    $($(this).find('.dd-list')[0]).find('li:last div').append('<div class="carousel-title">' +
//                        '<span>'+text+'</span> ' +
//                        '<i class="fa fa-times pull-right delete-element" style="cursor:pointer; margin-top:3px;"></i> ' +
//                        '<a href="javascript:void(0)" class="edit-block pull-right" data-block-id="'+avaiableBlocks+'">' +
//                        '   <i class="fa fa-pencil"></i>' +
//                        '</a>' +
//                        '</div>' +
//                        '<div id="'+$(event.target).attr('id')+'-carousels-container-'+last_index+'" class="carousels-container ui-state-default"  style=" border: 1px solid #D4D4D4">' +
//                        '   <ol class="dd-list" style="min-height:50px;"></ol>' +
//                        '</div>' +
//                        '<div class="text-center"><a onclick="MooLayout.setCustomElOb($(this))" href="'+url.admin_add_elements+'/'+currentPage+'" data-toggle="modal" data-target="#custom-elements" class="add-custom-elements fa fa-plus-square-o" style="cursor:pointer"></i></div>');
//                    var a = $($($(this).find('.dd-list')[0]).find('li:last div').find('a')[0]);
//                }
//                else//drop normal widget
//                {
                    $(this).find(".placeholder").remove();
                    droppedWidget = $(ui.draggable.clone()).appendTo($(this).find('.dd-list')[0]);
...
//                }
    ...
    var initPageInfo=function(){
        ...
        $.getJSON(short_url,function(data){
            ...
            console.log("Error when calling " + short_url)
        });
    ...
    var getRegionInfo = function(container){
        ...
        $.each(container,function(key,value){
            ...
            if($(value).attr('data-display-name') == 'tabsWidget')
            {
                ...
                {
                    console.log(Object.keys(tabsEl).length);
    ...
    var doSavePageContent = function(){
        ...
        $.ajax({
            ...
            async: true,
            url: short_url,
            data: {pageId: currentPage,columnStyle:column_layout,data:data},
            dataType: 'json',
    ...
    var showEditContent = function(blockId,node_ol,this_ob){
        ...
        $.getJSON(short_url,function(data){
            ...
            console.log('Had error with '+short_url);
        });
    ...
    var addImgEl = function (params){
        ...
        //var dataLength = Object.keys(editBlock.data()).length;
        //var startingInput = Math.floor(dataLength/2);
        var startingInput = 0;
...
//    });
//
//    $('#sidebar-right .draggable').draggable({
//        revert: true,
////proxy: 'clone'
//        proxy: function (source) {
//            var p = $('<div style="border:1px solid #ccc;width:80px"></div>');
...
//
//
//    $('.admin_layoutbox ul.drop').droppable({
//        onDragEnter: function () {
//            $(this).parent('td').addClass('over');
//        },
//        onDragLeave: function () {
//            $(this).parent('td').removeClass('over');
//        },
