...
class OAuth1Client{
	...
	*/ 
	function request( $url, $method, $postfields = NULL, $auth_header = NULL, $content_type = NULL )
	{
		...
		// SocialIntegration_Logger::debug( "OAuth1Client::request(). dump request info: ", serialize( curl_getinfo($ci) ) );
		// SocialIntegration_Logger::debug( "OAuth1Client::request(). dump request result: ", serialize( $response ) );
